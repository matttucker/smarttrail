...
public class SmartTrailDatabase extends SQLiteOpenHelper {
	...
	interface Tables {
		...
		// returns all trails even if area does not exist
		String TRAILS_JOIN_AREAS = TRAILS
				+ " LEFT OUTER JOIN areas ON trails.area_id=areas.id";

		// String TRAILS_AREAS_JOIN_AREAS = "trails_areas "
		// + "LEFT OUTER JOIN areas ON trails_areas.area_id=areas.id";
		//
	...
	public void onCreate(SQLiteDatabase db) {
		...
		db.execSQL("CREATE TABLE "
				...
				+ AreasColumns.BBOX_LAT_0 + " REAL," 
				+ AreasColumns.BBOX_LON_0 + " REAL," 
				+ AreasColumns.BBOX_LAT_1 + " REAL," 
				+ AreasColumns.BBOX_LON_1 + " REAL," 
				+ AreasColumns.VERSION + " INT,"
				+ "UNIQUE (" + AreasColumns.AREA_ID + ") ON CONFLICT REPLACE)");

		db.execSQL("CREATE TABLE " + Tables.TRAILS + " (" + BaseColumns._ID
				+ " INTEGER PRIMARY KEY AUTOINCREMENT,"
				+ TrailsColumns.TRAIL_ID + " TEXT NOT NULL,"
				+ TrailsColumns.AREA_ID + " TEXT NOT NULL,"
				+ TrailsColumns.MAP + " TEXT," 
				+ TrailsColumns.TRAILHEADS + " TEXT," 
				...
				+ TrailsColumns.DIFFICULTY_RATING + " INT,"
				+ TrailsColumns.TECH_RATING + " INT,"
				+ TrailsColumns.AEROBIC_RATING + " INT,"
				+ TrailsColumns.COOL_RATING + " INT," 
				+ TrailsColumns.IMAGE_URL + " TEXT," 
				+ TrailsColumns.KEYWORDS+ " TEXT," 
				+ TrailsColumns.STARRED+ " INTEGER NOT NULL DEFAULT 0," 
				+ TrailsColumns.BBOX_LAT_0 + " REAL," 
				+ TrailsColumns.BBOX_LON_0 + " REAL," 
				+ TrailsColumns.BBOX_LAT_1 + " REAL," 
				+ TrailsColumns.BBOX_LON_1 + " REAL," 
				+ "UNIQUE ("
				+ TrailsColumns.TRAIL_ID + ") ON CONFLICT REPLACE)");

		db.execSQL("CREATE TABLE " + Tables.POIS + " (" + BaseColumns._ID
				+ " INTEGER PRIMARY KEY AUTOINCREMENT," 
				+ PoisColumns.REGION_ID+ " TEXT NOT NULL," 
				+ PoisColumns.AREA_ID + " TEXT NOT NULL,"
				+ PoisColumns.TRAIL_ID + " TEXT NOT NULL," 
				+ PoisColumns.INDEX + " INT," 
				+ PoisColumns.NAME + " TEXT,"
				+ PoisColumns.DESCRIPTION + " TEXT,"
				+ PoisColumns.LAT + " REAL," 
				+ PoisColumns.LON + " REAL,"+ " TEXT," 
				+ "UNIQUE ("
				+ PoisColumns.TRAIL_ID+","
				+ PoisColumns.INDEX
				+ ") ON CONFLICT REPLACE)");

		db.execSQL("CREATE TABLE " + Tables.CONDITIONS + " (" + BaseColumns._ID
				+ " INTEGER PRIMARY KEY AUTOINCREMENT,"
				...
				+ ConditionsColumns.CONDITION + " TEXT,"
				+ ConditionsColumns.COMMENT + " TEXT,"
				+ ConditionsColumns.UPDATED_AT + " INT," + "UNIQUE ("
	...
	private static void createTrailsSearch(SQLiteDatabase db) {
		...
		db.execSQL("CREATE TRIGGER " + Triggers.TRAILS_SEARCH_UPDATE
				+ " AFTER UPDATE ON " + Tables.TRAILS + " BEGIN UPDATE "
				+ Tables.TRAILS_SEARCH + " SET " + TrailsSearchColumns.BODY
				+ " = " + Subquery.TRAILS_BODY + " WHERE "
				+ TrailsSearchColumns.TRAIL_ID + " = old."
				+ TrailsColumns.TRAIL_ID + " ; END;");
	...
	public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
		...
		if (version != DATABASE_VERSION) {
			...
			db.execSQL("DROP TABLE IF EXISTS " + Tables.POIS);
