...
import org.bouldermountainbike.smarttrail.provider.SmartTrailDatabase.Tables;
import org.bouldermountainbike.smarttrail.provider.SmartTrailDatabase.TrailsSearchColumns;
...
import org.bouldermountainbike.smarttrail.provider.SmartTrailSchema.SearchSuggest;
...
import org.bouldermountainbike.smarttrail.util.SelectionBuilder;

/**
 * Provider that stores {@link SmartTrailSchema} data. Data is usually inserted
...
public class SmartTrailProvider extends ContentProvider {
	...
	private static final int REGIONS_ID_TRAILS_JOIN_AREAS = 104;
	private static final int REGIONS_ID_POIS = 105;
	...
	private static final int POIS = 700;
	private static final int POIS_ID = 701;
	...
	private static UriMatcher buildUriMatcher() {
		...
		matcher.addURI(authority, "regions/*/trailsareas",
				REGIONS_ID_TRAILS_JOIN_AREAS);
		matcher.addURI(authority, "regions/*/pois", REGIONS_ID_POIS);
	...
	public String getType(Uri uri) {
		...
		case POIS:
			return PoisSchema.CONTENT_TYPE;
		case POIS_ID:
			return PoisSchema.CONTENT_ITEM_TYPE;
		case TRAILS_STARRED:
			return TrailsSchema.CONTENT_TYPE;
		case TRAILS_SEARCH:
			return TrailsSchema.CONTENT_TYPE;
	...
	public Uri insert(Uri uri, ContentValues values) {
		...
		case POIS: {
			long id = db.insertOrThrow(Tables.POIS, null, values);
			getContext().getContentResolver().notifyChange(uri, null);
			return PoisSchema.buildUri(id);
		}
	...
	private SelectionBuilder buildSimpleSelection(Uri uri) {
		...
		case POIS: {
			return builder.table(Tables.POIS);
		}
		case POIS_ID: {
			final long poiId = PoisSchema.getPoiId(uri);
			return builder.table(Tables.POIS).where(BaseColumns._ID + "=?",
					Long.toString(poiId));
		}
		case EVENTS: {
	...
	private SelectionBuilder buildExpandedSelection(Uri uri, int match) {
		...
		case POIS: {
			return builder.table(Tables.POIS).mapToTable(PoisSchema._ID,
					Tables.POIS);
...
//					.where(TrailsSchema.REGION_ID + "='" + regionId + "'");
		...
		case REGIONS_ID_POIS: {
			final String regionId = RegionsSchema.getRegionId(uri);
			return builder.table(Tables.POIS)
					.mapToTable(PoisSchema._ID, Tables.POIS)
					.where(PoisSchema.REGION_ID + "='" + regionId + "'");
		}
		case REGIONS_ID_TRAILS_JOIN_AREAS: {
			final String regionId = RegionsSchema.getRegionId(uri);
			return builder.table(Tables.TRAILS_JOIN_AREAS);
//					.where("trails." + TrailsSchema.REGION_ID + "='" + regionId
//							+ "'");
		}
...
//		}
	...
	/**
	 * {@link SmartTrailSchema} fields that are fully qualified with a specific
