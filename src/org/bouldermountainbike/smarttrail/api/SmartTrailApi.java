...
import static org.bouldermountainbike.smarttrail.api.SmartTrailApiV1.ERROR_DETAIL;
import static org.bouldermountainbike.smarttrail.api.SmartTrailApiV1.HTTP_CODE;
import static org.bouldermountainbike.smarttrail.api.SmartTrailApiV1.HTTP_CODE_SUCCESS;
import static org.bouldermountainbike.smarttrail.api.SmartTrailApiV1.META;
import static org.bouldermountainbike.smarttrail.api.SmartTrailApiV1.STATUS;
...
public class SmartTrailApi {
	...
	/**
	 * Signs the user into the server and returns a signin status code. Used to
	 * verify the user has valid credentials
	 * @return Signin result code. (fail, success)
	 * @throws IOException
	 * @throws SmartTrailException
	 * @throws CredentialsException
	 * @throws JSONException
	 */
	@api(version = "1")
	public JSONObject userCheck(String email) throws IOException, SmartTrailException,
	CredentialsException, JSONException {
		
		String json = mApiV1.userCheck(email);
		
		JSONObject result = new JSONObject(json);
		JSONObject meta = result.getJSONObject(META);
		if (meta.getInt(HTTP_CODE) == HTTP_CODE_SUCCESS) {
			return result.getJSONObject(SmartTrailApiV1.RESPONSE)
			.getJSONObject(SmartTrailApiV1.USER);
		} else {
			String errorDetail = "";
			if (meta.has(ERROR_DETAIL)) {
				errorDetail = meta.getString(ERROR_DETAIL);
	 ...
	 */
	@api(version = "1")
	public JSONObject pushCondition(String username, String email, Condition condition)
			throws CredentialsException, IOException, SmartTrailException,
			JSONException {
		String jsonResult = mApiV1.pushCondition(username, email,
				condition.mTrailId, condition.mStatus, 
				condition.mComment);
		...
		if (meta.getInt(HTTP_CODE) == HTTP_CODE_SUCCESS) {
			return result.getJSONObject(SmartTrailApiV1.RESPONSE)
					.getJSONArray(SmartTrailApiV1.CONDITION).getJSONObject(0);
		} else {
			String errorDetail = "";
			if (meta.has(ERROR_DETAIL)) {
				errorDetail = meta.getString(ERROR_DETAIL);
	 ...
	 */
	@api(version = "1")
	public ArrayList<Condition> getConditionsByTrail(String trailId, long afterTimestamp,
		...
		if (meta.getInt(HTTP_CODE) == HTTP_CODE_SUCCESS) {
			JSONArray json = result.getJSONObject(SmartTrailApiV1.RESPONSE).getJSONArray(
					SmartTrailApiV1.CONDITIONS);
			ArrayList<Condition> conditions = new ArrayList<Condition>();
			for (int i =0; i < json.length(); i++) {
				conditions.add(new Condition(json.getJSONObject(i)));
			}
			return conditions;
		} else {
			String errorDetail = "";
	 ...
	 */
	@api(version = "1")
	public JSONObject getTrailStatus(String trailId)
			throws CredentialsException, IOException, SmartTrailException,
			JSONException {
		String jsonResult = mApiV1.getTrailStatus(trailId);
		JSONObject result = new JSONObject(jsonResult);
		JSONObject meta = result.getJSONObject(META);
		...
		if (meta.getInt(HTTP_CODE) == HTTP_CODE_SUCCESS) {
			...
			if (response.isNull(STATUS)) {
				...
				// mkt: this should be an object, not an array
				return response.getJSONArray(STATUS).getJSONObject(0);
			}
